...
function errorMessage(status: number, payload: unknown): string {
  ...
  if (status === 400) return "Invalid phone number or code. Check and try again.";
  if (status === 401) return "Verification failed. Check the code and try again.";
...
export default function PhoneSignIn({ ageConfirmed, onAuthenticated }: PhoneSignInProps) {
  ...
  async function handleSend(event: FormEvent<HTMLFormElement>) {
    ...
    if (pending) return;
    if (!ageConfirmed) {
      setError("Please confirm you are 18 or older before signing in.");
      setStatus(null);
      return;
    }
