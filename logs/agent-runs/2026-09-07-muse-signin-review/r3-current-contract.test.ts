...
describe('Independent long-horizon review — real local database counterexamples', () => {
  ...
  it('records genuine age confirmation for an existing token-provisioned user', async () => {
    ...
    const response = await createHandler(post('/api/support-requests', unconfirmed, { topic: 'Work & Career Stress', language: 'English', ageConfirmed: true, serviceTier: 'listener', idempotencyKey: crypto.randomUUID() }));
